/*
 * Dunit OS - QEMU ramfb Display Driver
 *
...
int ramfb_setup(uint64_t fb_addr, uint32_t width, uint32_t height,
  ...
  dma.control = bswap32(((ramfb_selector | FW_CFG_WRITE_CHANNEL) << 16) |
                        0x08 | 0x10);
...
#ifdef ARCH_X86_64
  outl(FW_CFG_IO_DMA, bswap32((uint32_t)(dma_addr >> 32)));
  outl(FW_CFG_IO_DMA + 4, bswap32((uint32_t)dma_addr));
#else
  ...
  *dma_reg = bswap64(dma_addr);
  ...
  if (bswap32(dma.control) & 0x01) {
    printk(KERN_ERR "RAMFB: DMA write failed\n");
    return -1;
  }
