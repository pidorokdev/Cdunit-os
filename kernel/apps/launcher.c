...
static const char *stub_cpu_name(void) { return "Dunit virtual CPU"; }
...
void kapi_init(kapi_t *api) {
    ...
    /* Console I/O - in order per dunit.h */
    api->putc = kapi_putc;
    api->puts = kapi_puts;
...
static int sysmon_app_main(kapi_t *api, int argc, char **argv) {
    ...
    api->puts("\n=== Dunit OS System Monitor ===\n\n");
    
    /* Display system information */
    api->puts("SYSTEM INFO\n");
    api->puts("-----------\n");
    api->puts("OS:       Dunit OS v0.5.0\n");
    api->puts("Arch:     multiarch desktop target\n");
    api->puts("Platform: QEMU Green Tea session\n\n");
