/*
 * Dunit OS Kernel - Main Entry Point
 *
...
/* Kernel version */
#define DUNITOS_VERSION_MAJOR 0
#define DUNITOS_VERSION_MINOR 5
#define DUNITOS_VERSION_PATCH 0
...
static void print_banner(void) {
  printk("\n");
  printk("=== Dunit OS Boot Sequence ===\n");
  printk("        ____              _ _   \n");
  printk("       |  _ \\ _   _ _ __ (_) |_ \n");
  printk("       | | | | | | | '_ \\| | __|\n");
  printk("       | |_| | |_| | | | | | |_ \n");
  printk("       |____/ \\__,_|_| |_|_|\\__|\n");
  printk("\n");
  printk("[ .. ] Starting Dunit OS (Green Tea)\n");
  printk("Dunit OS v%d.%d.%d - %s desktop branch\n", DUNITOS_VERSION_MAJOR,
         DUNITOS_VERSION_MINOR, DUNITOS_VERSION_PATCH, ARCH_NAME);
  printk("Terminal-first hobby operating system for %s\n", ARCH_NAME);
  printk("Copyright (c) 2026 Dunit OS Project\n");
  printk("\n");
}
...
static void init_subsystems(void *dtb) {
  ...
  ramfs_create_file("/Desktop/notes.txt", 0644,
                    "Dunit OS Green Tea session\n\nroot@dunit:~# dufetch\n");
  ramfs_create_file("/Desktop/readme.txt", 0644,
                    "Dunit OS desktop branch\n\n/app contains launchers\n/assets "
                    "contains the imported Green Tea artwork\n");
  ...
  ramfs_create_file("todo.txt", 0644,
                    "- Polish Green Tea desktop\n- Wire more /app launchers\n- "
                    "Keep tty fast");
  ramfs_create_file_bytes("sample.mp3", 0644, dunit_seed_mp3, dunit_seed_mp3_len);
  ...
  ramfs_create_file_bytes("Pictures/gora.bmp", 0644, dunit_wallpaper_gora_bmp,
                          dunit_wallpaper_gora_bmp_len);
  ramfs_create_file_bytes("Pictures/stena.bmp", 0644, dunit_wallpaper_stena_bmp,
                          dunit_wallpaper_stena_bmp_len);
  ramfs_create_file_bytes("Pictures/wallpaper.bmp", 0644,
                          dunit_wallpaper_wallpaper_bmp,
                          dunit_wallpaper_wallpaper_bmp_len);
  ...
  extern int gui_take_redraw_request(void);
