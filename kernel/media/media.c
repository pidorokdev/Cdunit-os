/*
 * Dunit OS - Media helpers (JPEG/MP3 decoding)
 */
...
static uint16_t media_le16(const uint8_t *p) {
  return (uint16_t)p[0] | ((uint16_t)p[1] << 8);
}

static uint32_t media_le32(const uint8_t *p) {
  return (uint32_t)p[0] | ((uint32_t)p[1] << 8) | ((uint32_t)p[2] << 16) |
         ((uint32_t)p[3] << 24);
}

static int32_t media_le32s(const uint8_t *p) {
  return (int32_t)media_le32(p);
}

int media_decode_bmp_buffer(const uint8_t *data, size_t size,
                            media_image_t *out, uint32_t *buffer,
                            size_t buffer_size) {
  if (!data || size < 54 || !out)
    return -EINVAL;
  ...
  uint32_t pixel_offset = media_le32(data + 10);
  uint32_t dib_size = media_le32(data + 14);
  if (dib_size < 40 || pixel_offset >= size)
    return -EINVAL;

  int32_t width_s = media_le32s(data + 18);
  int32_t height_s = media_le32s(data + 22);
  uint16_t planes = media_le16(data + 26);
  uint16_t bpp = media_le16(data + 28);
  uint32_t compression = media_le32(data + 30);

  if (width_s <= 0 || height_s == 0 || planes != 1 || bpp != 24 ||
      compression != 0) {
    printk(KERN_ERR "BMP: unsupported format\n");
    return -EINVAL;
  }

  uint32_t width = (uint32_t)width_s;
  uint32_t height = height_s < 0 ? (uint32_t)(-height_s) : (uint32_t)height_s;
  ...
  if (pixel_offset + row_stride * height > size)
    return -EINVAL;
  ...
  for (uint32_t y = 0; y < height; y++) {
    uint32_t src_y = height_s < 0 ? y : (height - 1 - y);
    const uint8_t *row = data + pixel_offset + (size_t)src_y * row_stride;
...
int media_decode_bmp(const uint8_t *data, size_t size, media_image_t *out) {
  return media_decode_bmp_buffer(data, size, out, NULL, 0);
}
