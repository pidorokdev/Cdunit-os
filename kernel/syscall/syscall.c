/*
 * Dunit OS Kernel - System Call Implementation
 */
...
static inline uint64_t pt_regs_get_pc(const struct pt_regs *regs) {
  return regs->pc;
}
...
void handle_sync_exception(struct pt_regs *regs) {
  ...
  case 0x21: /* Instruction abort from same EL */
    printk(KERN_EMERG "Instruction abort at PC=0x%llx\n",
           (unsigned long long)pt_regs_get_pc(regs));
