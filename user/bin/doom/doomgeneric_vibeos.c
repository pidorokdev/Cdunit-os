...
/* Map DunitOS key to DOOM key */
static unsigned char translate_key(int dunit_key) {
    /* Arrow keys */
    if (dunit_key == 0x100) return KEY_UPARROW;     /* KEY_UP */
    if (dunit_key == 0x101) return KEY_DOWNARROW;   /* KEY_DOWN */
    if (dunit_key == 0x102) return KEY_LEFTARROW;   /* KEY_LEFT */
    if (dunit_key == 0x103) return KEY_RIGHTARROW;  /* KEY_RIGHT */

    /* Modifier keys */
    if (dunit_key == 0x109) return KEY_RCTRL;       /* SPECIAL_KEY_CTRL = fire */
    if (dunit_key == 0x10A) return KEY_RSHIFT;      /* SPECIAL_KEY_SHIFT = run */

    /* Special keys */
    if (dunit_key == 27) return KEY_ESCAPE;
    if (dunit_key == '\n' || dunit_key == '\r') return KEY_ENTER;
    if (dunit_key == '\t') return KEY_TAB;
    if (dunit_key == ' ') return KEY_USE;           /* Space = use/open doors */
    if (dunit_key == 127 || dunit_key == 8) return KEY_BACKSPACE;

    /* Control key (ASCII 1-26 are Ctrl+letter) - also fire */
    if (dunit_key >= 1 && dunit_key <= 26) {
        return KEY_RCTRL;  /* Ctrl+letter = fire */
    }

    /* Function keys (if supported) */
    if (dunit_key >= 0x110 && dunit_key <= 0x11B) {
        return KEY_F1 + (dunit_key - 0x110);
    }

    /* WASD movement + E for use */
    if (dunit_key == 'w' || dunit_key == 'W') return KEY_UPARROW;
    if (dunit_key == 's' || dunit_key == 'S') return KEY_DOWNARROW;
    if (dunit_key == 'a' || dunit_key == 'A') return KEY_STRAFE_L;
    if (dunit_key == 'd' || dunit_key == 'D') return KEY_STRAFE_R;
    if (dunit_key == 'e' || dunit_key == 'E') return KEY_USE;

    /* Letters - lowercase them for DOOM */
    if (dunit_key >= 'A' && dunit_key <= 'Z') {
        return dunit_key + 32;  /* lowercase */
    }
    if (dunit_key >= 'a' && dunit_key <= 'z') {
        return dunit_key;
    }
    ...
    if (dunit_key >= '0' && dunit_key <= '9') return dunit_key;
    if (dunit_key == '-') return KEY_MINUS;
    if (dunit_key == '=') return KEY_EQUALS;
    if (dunit_key == '+') return '+';
    if (dunit_key == ',') return ',';
    if (dunit_key == '.') return '.';
    if (dunit_key == '/') return '/';
    ...
    /* Y/N for prompts */
    if (dunit_key == 'y' || dunit_key == 'Y') return 'y';
    if (dunit_key == 'n' || dunit_key == 'N') return 'n';
...
static void poll_keys(void) {
     ...
     * (DunitOS doesn't have key-up events, so we fake them) */
...
void DG_Init(void) {
    ...
    printf("DG_Init: DunitOS DOOM initialized (FULLSCREEN 2x)\n");
...
uint32_t DG_GetTicksMs(void) {
    /* DunitOS ticks are 10ms each (100Hz timer) */
    uint64_t now = doom_kapi->get_uptime_ticks();
...
void DG_SetWindowTitle(const char *title) {
    /* No window title in DunitOS - just print to console */
    (void)title;
}
