...
static inline void dunit_print_uint(kapi_t *k, unsigned int n) {
    if (n == 0) {
        dunit_putc(k, '0');
        return;
    }
...
static inline void dunit_print_hex(kapi_t *k, uint32_t n) {
    if (n == 0) {
        dunit_putc(k, '0');
        return;
    }
...
static inline void dunit_print_size(kapi_t *k, uint64_t bytes) {
    if (bytes < 1024) {
        dunit_print_uint(k, (unsigned int)bytes);
        dunit_puts(k, " B");
    } else if (bytes < 1024 * 1024) {
        dunit_print_uint(k, (unsigned int)(bytes / 1024));
        dunit_puts(k, " KB");
    } else if (bytes < 1024ULL * 1024 * 1024) {
        dunit_print_uint(k, (unsigned int)(bytes / (1024 * 1024)));
        dunit_puts(k, " MB");
    } else {
        dunit_print_uint(k, (unsigned int)(bytes / (1024ULL * 1024 * 1024)));
        dunit_puts(k, " GB");
    }
}
